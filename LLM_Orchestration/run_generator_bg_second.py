...
def main():
    ...
    # --- Selective Cleanup ---
    # Kill old generator processes for this specific script, but exclude our own PID
    my_pid = os.getpid()
    ...
    print(f"{'='*60}")
    print(f" Dataset Generator Background Manager (Instance 2)")
    print(f"{'='*60}")
    ...
    with open(GENERATOR_LOG, "a") as generator_handle:
        ...
        generator_proc = start_process([PYTHON_EXEC, str(GENERATOR_SCRIPT)], generator_handle, "Dataset Generator (Second)", cwd=BASE_DIR)
